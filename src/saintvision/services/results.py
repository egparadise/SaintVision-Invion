...
# --------------------------------------------------------------------------
# Downloading what a Run actually produced
# --------------------------------------------------------------------------


def committed_outputs(
    session: Session, *, tenant_id: uuid.UUID, run_id: str, user_id: str
) -> list[dict[str, Any]]:
    """The output objects the kernel committed for this Run.

    Not ``public.artifacts``. Nothing writes that table and a row in it cannot be
    resolved to a file, so listing it would offer downloads that cannot happen.
    What exists is ``inv.result_commitments`` — a Run, an attempt, an object and
    the Evidence that recorded it — reached through a definer function scoped to
    the session's tenant (migration 0029).
    """
    run = session.get(Run, run_id)
    if run is None or run.tenant_id != tenant_id:
        raise InvError(RES_NODE_NOT_FOUND, "run not found")
    ...
    rows = session.execute(
        text(
            "SELECT object_id, content_hash, size_bytes, state, evidence_id, "
            "attempt, prepared_at FROM public.run_committed_outputs(:t, :r)"
        ),
        {"t": str(tenant_id), "r": run_id},
    ).all()
    return [
        {
            ...
            "evidenceId": row[4],
            "attempt": row[5],
            "preparedAt": row[6].isoformat(),
            # Only a ready object has bytes to serve. Offering a download for an
            # uploading or deleted one produces a failure at the moment somebody
            # clicks, which is the worst time to discover it.
            "downloadable": row[3] == "ready",
        }
        for row in rows
    ]
...
def read_output(
    ...
    *,
    ...
    """Read one committed output, verifying it on the way out.

    The bytes come back through ``inv.object_store.LocalObjects``, which
    re-hashes what it read and compares against the recorded digest before
    returning. That check is the point: a file that has drifted from the digest
    the Evidence recorded must fail the download rather than be served, because
    serving it makes the Evidence a lie about something somebody now has a copy
    of.

    The object key is derived from the object id rather than taken from the
    request. A caller-supplied key is a path, and a path from a caller is how a
    store gets read outside itself.
    """
    outputs = committed_outputs(
        session, tenant_id=tenant_id, run_id=run_id, user_id=user_id
    )
    match = next((o for o in outputs if o["objectId"] == str(object_id)), None)
    if match is None:
        # The same answer whether it belongs to another Run or does not exist.
        raise InvError(RES_NODE_NOT_FOUND, "output not found for this run")
    if match["state"] != "ready":
        raise InvError(
            VAL_SCHEMA,
            f"this output is {match['state']}, so it has no bytes to read",
            extra={"objectId": match["objectId"], "state": match["state"]},
        )

    from inv.object_store import LocalObjects

    key = "obj-" + uuid.UUID(match["objectId"]).hex
    with LocalObjects(object_root).locked() as handle:
        data = handle.read(key, match["contentSha256"], match["sizeBytes"])
    return data, match
