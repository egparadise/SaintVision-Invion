...
title: "화면 개발 정직성 7대 원칙 및 사례집 (자동화 가능 vs 사람 판단, 감사 지도)"
version: "1.1.0"
status: "active"
author: "Gemini"
reviewer: "Codex"
source_of_truth: "Git"
updated: "2026-09-22"
tags: ["governance", "frontend", "integrity", "audit", "discovery", "exists-vs-works", "freshness", "unimplemented-consistency"]
---

# 화면 개발 정직성 7대 원칙 및 사례집
...
### 원칙 ⑥: 시간의 신선도와 출처를 왜곡하지 않는다 (Never Forge Timestamps & Clarify Freshness)
화면에 표시되는 시간 정보는 서버가 기록한 진실 시각(Truth Time)과 클라이언트의 관측 시각(Query Time)을 엄격히 구별해야 하며, 서버가 시각을 주지 않았다고 해서 브라우저 현재 시각(`new Date().toISOString()`)으로 지어내지 않는다. 아울러 시각 필드를 노출할 때는 그 시각이 **무엇의 시각이며 무엇이 아닌지**를 명확히 한정 고지해야 한다.

- **사례 1: `DeveloperStudio.tsx` 아티팩트 내보내기 현재 시각 합성 제거**
  - **결함**: 서버 응답에 완료 시점(`completedAt`)이 부재할 때 `res.completedAt || new Date().toISOString()`으로 현재 시각을 합성하여 마치 방금 서버가 완료한 것처럼 타임스탬프를 위조함.
  - **정정**: 현재 시각 fallback을 완전 소거하고 `res.completedAt || null`로 정직하게 유지.
- **사례 2: `RunDetail.tsx`의 `completedAt` 및 `stateAsOf` 오독 방지 한정 고지**
  - **결함**: `completedAt`을 화면 마지막 확인 시각이나 실시간 수집 시각으로 오인하거나, `stateAsOf`를 클러스터 전체가 동시에 관측된 단일 공통 스냅샷으로 오해할 위험 존재.
  - **정정**: 고유 라벨(실행 완료 시각, 샤드 상태 기준)을 부여하고 "(커널 결과 완료 커밋 시각이며 실시간 로그 캡처나 파일 다운로드 시각이 아닙니다)", "(포함된 Run 행들의 최신 DB 갱신 시각 기준이며, 단일 공통 스냅샷이나 조회 시각이 아닙니다)"라는 부인 고지(Disclaimer)를 명시하여 오독 차단.

---

### 원칙 ⑦: 있는 것을 없다고 하거나 배선해 두고 미구현이라 속이지 않는다 (Honest Capability & Unimplemented Consistency)
없는 기능을 있다고 하는 것뿐만 아니라, **있는 기능을 없다고 하는 것(허위 미구현)**도 중대한 결함이다. 있는 기능을 없다고 표시하면 사용자가 유효한 기능을 시도조차 하지 않게 되어 결함이 영구히 은폐된다. 특히 실제 백엔드 라우트나 API 클라이언트 함수가 배선되어 있음에도 화면 텍스트로 "API 미노출", "미구현"이라고 주장하는 모순을 금지한다.

- **사례 1: `RunDetail.tsx` 산출물 다운로드 실배선 및 허위 미노출 공지 소거**
  - **결함**: 백엔드 커널에 실제 산출물 스트림 엔드포인트(`GET /v1/projects/.../artifacts/content`)가 실재함에도, fallback 목록의 버튼 텍스트에 `다운로드 (API 미노출)`을 붙이고 `[모의 고지] ... (서버 아티팩트 파일 스트림 다운로드 API 미노출 상태)` 배너를 띄움.
  - **정정**: `getArtifactDownloadUrl`을 통한 실제 다운로드 링크(`<a>` 태그, download 속성)로 실배선하고 `[모의 고지]` 및 `(API 미노출)` 허위 텍스트를 전면 소거하여 `📥 다운로드`로 통일.
- **사례 2: `MonacoWorkspaceEditor.tsx` 저장 API 실재와 컨텍스트 부재의 구별**
  - **결함**: 백엔드에 체크아웃 파일 저장 엔드포인트(`saveWorkspaceEditView`)가 실재함에도, 에디터 상단 공지와 툴팁에 `(백엔드 저장 API 미노출)`이라고 일괄 표기함.
  - **정정**: "백엔드 저장 API 미노출"이 아닌 "체크아웃 컨텍스트 미연결 (로컬 인메모리 버퍼 샌드박스 · 커널 영속 저장 시 runId/checkoutId 필요)"로 정정하여 기능 부재와 컨텍스트 미충족을 엄격히 구별.
...
| **자동화 가능 (구조적)** | 타임스탬프 현재 시각 합성(`|| new Date().toISOString()`) 및 신선도 한정 고지 부재 탐지 | `tools/check_frontend_integrity.py` [RULE-6] | 정적으로 라벨과 디스클레이머가 붙어 있더라도 실제 DB 기록 시각과의 클록 스큐는 런타임 검증 필요. |
| **자동화 가능 (구조적)** | 배선-미구현 모순(API 바인딩 존재하면서 미노출 주장) 및 허위 미노출 공지 차단 | `tools/check_frontend_integrity.py` [RULE-7] | 화면의 '미제공'이 실제 백엔드 라우트 부재인지 프론트엔드의 오판인지는 백엔드 라우트 테이블 대조 필요. |
...
### 3.2 검사 도구의 구조적 한계 (스캐너가 못 잡는 것과 사람 판단의 영역)

> [!WARNING]
> **검사의 초록(0 violations)이 완벽한 안전을 뜻하지 않는다.**
> 자동화 검사는 정적 텍스트 구조의 하한선(바닥)을 올릴 뿐이며, 검사가 감지할 수 없는 구조적 한계를 명확히 인지하지 못하면 "검사를 통과했으니 안전하다"는 허위 안도감(False Sense of Security)에 빠지게 된다.

`tools/check_frontend_integrity.py`가 구조적으로 **잡지 못하는 9가지 한계**는 다음과 같으며, 이는 독립 검토와 런타임 E2E 테스트가 감당해야 할 사람 판단의 영역이다:

1. **변수 조립 및 계산식으로 생성한 가짜 식별자 (Dynamic / Computed Values)**:
   - 스캐너는 정적 문자열 리터럴 패턴을 검사한다.
   - 문자열 연결(`'00000000-' + '0000-...'`), 템플릿 리터럴(`` `run_${computedId}` ``), 또는 난수/해시 함수로 런타임에 합성된 가짜 식별자는 정규식 스캐너를 우회할 수 있다.
2. **오늘 목록에 없는 신규 형태의 합성 식별자 (Unregistered Novel Patterns)**:
   - 본 도구는 오늘 발견된 위반의 지문(`00000000-0000-...`, `run_01J...`, `apr_01J...`, `prj_01J...` 등)을 박아놓은 것이다.
   - 향후 개발자가 완전히 새로운 명명 체계(예: `mock_node_9999`, `test_tenant_xyz`)로 가짜 데이터를 주입하면, 스캐너의 `FORBIDDEN_PLACEHOLDERS`에 등록되기 전까지는 포착할 수 없다.
3. **특정 컴포넌트를 이름으로 지정한 규칙의 미추적 (Targeted Component Scope vs New Files)**:
   - 규칙 4(엄격한 3상태)는 `InvFileExplorer.tsx`를 지목하고, 규칙 5(0-call 가드)는 `ResourceExplorer.tsx`의 특정 함수를 지목한다.
   - 다른 파일이나 향후 신설될 새 뷰(예: `DatasetExplorer.tsx` 또는 `ModelRegistryView.tsx`)에서 유사한 해시 검증이나 안내 방송을 구현할 경우, 파일명이 하드코딩된 규칙은 자동으로 확장 적용되지 않는다.
4. **소스 어휘의 존재 vs 런타임 데이터 흐름 (Lexical Syntax vs Runtime Data Flow)**:
   - 스캐너는 `!tenantId` 조건문이 소스 코드에 존재하는지만 확인하며, 실제로 검증된 `tenantId`가 API 요청의 `X-Inv-Tenant` 헤더에 정확히 실려 날아가는지(Data Flow)는 추적하지 못한다.
   - 마찬가지로 `activeRunId || ""`에서 빈 문자열 fallback이 런타임 백엔드 스키마 유효성 검사에서 통과하는지 실패하는지는 런타임 통합 시험으로만 알 수 있다.
5. **모양만 유효한 임의 식별자의 실존성 (Syntactic Shape Validity vs Semantic Existence)**:
   - 금지 목록에 없는 임의의 정상 UUID(예: `d4e5f6a7-b8c9-...`)나 ULID는 스캐너를 그대로 통과한다.
   - 그 식별자가 실제로 백엔드 데이터베이스에 존재하는지, 현재 로그인한 테넌트 소유인지, 아니면 다른 테넌트의 자원인지는 백엔드 fail-closed 인가 계층과 DB 대조 시험만이 증명할 수 있다.
6. **비-텍스트적/시각적 조기 성공 표출 (Visual / Non-Textual Premature Success)**:
   - 스캐너는 `'Connected via secure WebSocket'`과 같은 특정 텍스트 키워드를 검출한다.
   - 텍스트 대신 녹색 점(Green Dot Indicator), 체크마크 SVG 아이콘, 또는 CSS 애니메이션 클래스(`is-connected`)를 소켓 연결 전에 미리 렌더링하는 시각적 조기 성공은 텍스트 정규식으로 감지할 수 없다.
7. **타임스탬프의 시간적 진실성과 클록 스큐 (Temporal Truth & Clock Skew)**:
   - 스캐너는 타임스탬프 필드에 클라이언트 현재 시각 합성이 없는지, 그리고 설명 라벨과 부인 고지가 존재하는지는 검사할 수 있다.
   - 그러나 그 시각이 실제 DB에 트랜잭션으로 커밋된 시각인지, 백엔드 분산 노드 간 클록 스큐(5초 허용 한도)가 발생했는지, 관측된 복제본 시각(`observedAt`)이 "재검증 필요(`currentHealth: unknown`)" 상태를 내포하는지는 정적 분석으로 알 수 없다. 이는 백엔드 진실 시각 감사와 DB 통합 시험(Codex PG16)의 몫이다.
8. **백엔드 라우트 실재성 vs 화면의 미구현 표기 (Backend Route Existence vs Client Label)**:
   - 화면에 "미제공 (API 미노출)" 또는 "시뮬레이션"이라고 표시되었을 때, 이것이 실제 백엔드 엔드포인트 부재(예: 분산 복구 `/v1/recovery/*` 부재, 물리 노드 실시간 사용량 HTTP 읽기 부재)에 대한 정직한 표기인지, 아니면 백엔드에 엔드포인트가 실재하는데 프론트엔드가 인지하지 못해 발생한 "허위 미구현(Class 3 결함)"인지는 백엔드 라우트 정의(`app.py`, OpenAPI 스키마)와 대조해야만 판정할 수 있다. (Claude의 백엔드 대조 감사 레인).
9. **수명주기 조건 미충족 / 대역외 안내의 미구현 둔갑 (Condition Guard vs Feature Absence)**:
   - 실행 진행 중(running)이라 아티팩트 파일이 아직 생성되지 않아 다운로드가 비활성화된 상태(수명주기 조건 미충족)이거나, 물리 노드에서 CLI를 실행해야 하는 대역외(Out-of-band) 온보딩 절차인 경우를 화면이 "기능 미구현"으로 오판하여 표기하는 것은 도메인 및 수명주기에 대한 이해가 필요하며 정적 문맥만으로는 판단할 수 없다.
