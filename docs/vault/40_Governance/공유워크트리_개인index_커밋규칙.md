...
- **R2-b push는 게이트의 exit code로 막는다 — 출력을 보는 것은 게이트가 아니다.** 착지 전 check_docs 등 게이트를 돌렸으면 그 **exit를 push 조건 안에 넣어라**: `CD=$?; ... ; if [ "$CD" -eq 0 ] && [ "$NOW" = "$BASE" ]; then git push ...`. **위반 = 게이트를 돌리고도 exit를 안 보고(또는 tip 비교로만 게이트하고) push하는 것.** "출력이 통과처럼 보였다"는 변명이 안 된다. 실례(2026-09-22, 2회): 문서에 위키링크 문법(메모리 슬러그·빈 대괄호 예시)을 넣어 check_docs가 exit 1이었는데 push를 tip 비교로만 게이트해 **red가 tip에 앉았다**(1d413155→d1c6992c, 44022523→2c83aa21). **R2-a와 형제**: R2-a는 drift 검사를 출력만 하고 안 멈춘 것, R2-b는 게이트를 돌려놓고 결과를 안 본 것. 둘 다 **「검사 ≠ 준수」**(검사는 했으나 그 결과가 다음 행동을 안 바꿈) 부류다 — 검사 결과는 반드시 **다음 명령의 조건**이어야 한다(printing ≠ gating).
  올바른 형태 (검사 exit가 다음 행동을 **실제로** 막는다 — 복사해서 쓴다). 세미콜론은 두 셸 다 게이트가 아니다(`a; b`는 a가 실패해도 b를 실행), 파이프는 앞 명령의 exit를 잃는다(`check | tail`은 tail의 exit).

  - **bash** — `if`나 `&&`로 검사 exit에 push를 건다:
    ```bash
    if python tools/check_docs.py; then git push origin "$C:refs/heads/<branch>"; else echo "BLOCKED: check failed"; fi
    # 요약 줄이 필요하면: 출력은 변수로 받고 exit는 따로 — 파이프에 넘기지 말 것
    out=$(python tools/check_docs.py); CD=$?; echo "$out" | tail -1; [ "$CD" -eq 0 ] || exit 1
    ```
  - **PowerShell** — `;`가 앞 실패를 무시하므로 네이티브 exe는 `$LASTEXITCODE`로 판정(파이프도 exit를 잃음):
    ```powershell
    python tools/check_docs.py
    if ($LASTEXITCODE -eq 0) { git push origin "${C}:refs/heads/<branch>" } else { "BLOCKED: check exit $LASTEXITCODE" }
    ```
