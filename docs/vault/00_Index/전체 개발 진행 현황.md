...
2026-09-21 sync 충돌 해소 도구: 현재 충돌 경로를 명시 목록으로만 선택하는 `--resolve-conflicts-from`을 추가했다. 임시 repo 13 passed 및 stale-path 검증 제거 되돌림 실패를 확인했다. 공유 vault 적용은 하지 않았고 사용자 14경로 목록 대기다. 부분 처리 시 잔여 충돌은 exit 3이며 해소 경로를 개별 출력한다. [[2026-09-21_sync_obsidian_explicit_conflict_resolution_Codex]]
