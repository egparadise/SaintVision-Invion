...
후속 correction 및 실행: `test_recovery_drill`의 기존 18 setup errors는 수정 전 기준이다. `aeec9b3` post-fix 실PG 실행에서 CX01_CONTAINER unset은 18 reasoned skips/0 error, 명시 owner는 12 본문 pass/4 Linux skip/2 archiver fixture readiness failures로 나뉘었다. 2개 준비 timeout은 root cause 미확정이며 전체 파일 clean pass가 아니다. full-suite JUnit 재실행 산출물은 아직 대기 중이다. Docker cleanup label inventory에서 빠진 rpo-test/rpo-network를 추가하고 17개 회귀를 통과했다. [[2026-09-19_recovery_drill_Docker_전제_skip_경계_Codex]]
