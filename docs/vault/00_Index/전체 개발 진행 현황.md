...
2026-09-21 sync EOL 비교 보강: `sync_obsidian.py`는 CRLF→LF 정규화 해시로만 비교하고 실제 파일 바이트는 보존한다. 정규화 회귀 8 passed; 683건 fixture 되돌림은 683으로 복귀해 시험 실패. 공유 vault `--check`는 현재 16건(14 no-baseline, 2 both-diverged), `--apply` 미실행. [[2026-09-21_sync_obsidian_state_and_static_markup_audit_Codex]]
