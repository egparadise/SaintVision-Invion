...
후속 correction: test_recovery_drill의 이전 18 setup errors는 CX01_CONTAINER identity 누락 또는 allowlist에 자체 codex-db-test owner label이 없었던 경우다. tests/recovery_drill_prerequisites.py는 identity/daemon 부재를 사유별 skip하고 존재하지만 미소유인 container는 failure로 처리한다. 오프라인 경계시험 11 passed; Claude 전체 회귀 종료 후 real PostgreSQL post-fix 실행 대기: [[2026-09-19_recovery_drill_Docker_전제_skip_경계_Codex]].
