...
> 충돌 시 [[최종 개발 계획 - 모든 개발의 지침]]과 [[설계 충돌 정정 및 ADR]]을 따릅니다. 이 문서는 원문 보존용이며 확정 구현 사양이 아닙니다. 원본은 Git `docs/sources/00_Index/SaintVision INV 개발 설계 인덱스.md`에 SHA-256과 함께 보존됩니다.
...
새 사용자 비전과 이후 일정은 [[2026-09-15 단일 가상 컴퓨터 보강 설계 인덱스]]를 진입점으로 삼는다. 여기에는 웹 단일 가상 컴퓨터, 통합 저장 namespace, 분산 모델 shard/replica, Agent별 후속 카드, 중간 사용자 확인 없이 최종 결과까지 이어가는 정책이 연결되어 있다.
