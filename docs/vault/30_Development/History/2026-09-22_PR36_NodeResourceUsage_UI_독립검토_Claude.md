...
| C4 | 스캐너 사각 재현 | `route_coverage.client_paths()`에 (a) PR의 어댑터 본문, (b) `apiClient(\`/v1/nodes/${id}/resource-usage\`)` 직접 리터럴 | (a) **∅**(못 잡음) / (b) `{'/v1/nodes/{}/resource-usage'}`(잡음). **원인 정정(v1.1.0)**: 처음엔 "변수에 조립한 URL"로 적었으나 스캐너는 파일 텍스트 전체를 훑으므로 조립 여부는 무관하다. 진짜 원인은 **hole 안의 함수 호출** — `${encodeURIComponent(nodeId)}`의 `(`·`)`가 리터럴 정규식의 허용 문자 밖이고, 대체 경로(head)는 `/v1/nodes/`로 끝나 prefix로 버려진다. 같은 모양이 apps/web에 15곳(13 경로: approvals/review·runs/result·logs·shards·terminals 등)이라 30/0은 그 전부를 못 본 숫자였다. 수정 PR: [[2026-09-22_route_coverage_템플릿hole_사각_수정_Claude]] |
...
- **F3 — route_coverage 스캐너 사각 (도구 소유 Claude; Rule 8 부류)**: 템플릿 hole 안에 함수 호출이 있으면 경로를 못 본다(C4; v1.0.0의 "변수 조립" 진단은 틀렸고 정정함). 그래서 G6이 30/0으로 초록이면서 F1을 놓쳤다 = **"검사는 했으나 대상 밖"**(오늘 밤 부류: 스캐너 모양지정). 스캐너 docstring이 "static path shapes only"라 자기 한계는 선언했으나, 이 모양(어댑터 함수 안 `const url = …; apiClient(url)`)은 코드베이스에 흔하다 → 지정 확장 필요. **내 레인 후속**: `client_paths`가 `apiClient(<식별자>)`의 정의를 같은 파일에서 역추적(또는 어댑터에서 리터럴을 직접 넘기는 관례 강제).
