...
- `provision_credentials.py`
  ...
  - 기존 missing DSN/manifest 거부의 exit 2 호환성을 유지하고, 내부 오류는 exit 3으로 구분한다.
...
- `plan_lan_migration.py`
  ...
  - fetchone shape 오류 등 내부 오류는 `migration_metadata_internal_error`, exit 3으로 구분한다.
...
`.venv/Scripts/python.exe -m pytest -q tests/core/test_credential_provision_cli.py tests/core/test_lan_migration_plan.py` → **16 passed / 1 skipped**. skip은 `INV_TEST_ADMIN_DSN` 부재에 따른 실제 PostgreSQL 시험 미실행이다.

DB 오류·내부 TypeError를 합성 주입해 category와 exit를 확인했다. 구현 원복 후 동일 신규 시험 4건은 모두 실패했다(기존 catch-all/일반 unavailable 메시지로 분류가 사라짐). 실제 PostgreSQL provisioning/migration 실행은 하지 않았다.
