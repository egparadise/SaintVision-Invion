...
1. `downloaded_real_uvicorn_artifact.bin`: Uvicorn 서버가 보낸 정확히 50바이트의 바이너리 (`verified` 갈래).
2. `real_chrome_real_uvicorn_verified.png`: Chrome 화면 상의 `[전송 확인 완료]` 배너 스크린샷.
3. `real_chrome_real_uvicorn_mismatch_blocked.png`: `[전송 불일치 · 저장 차단]` 배너 스크린샷 (`mismatch` 갈래).
4. `real_chrome_real_uvicorn_missing_header_blocked.png`: `[전송 헤더 누락 · 저장 차단]` 배너 스크린샷 (`missing-header` 갈래).
5. `chrome_real_uvicorn_acceptance_result.json`: 3대 갈래 전수 통과 레코드.

---

## 6. 3대 갈래 음성 대조 및 돌연변이 실측 사살 (Mutation Testing)

도구가 "초록인데 아무것도 잡지 못하는 가짜 통과"가 아님을 입증하기 위해, 3대 갈래 각각에 대해 의도적 결함을 주입하여 도구가 이를 실측 사살(KILLED, exit 1)하는지 검증을 완료했습니다:

| 갈래 | 주입 결함 (돌연변이) | 도구 실측 결과 | 복구 실측 |
|---|---|---|---|
| **갈래 1 (`verified`)** | 정상 일치 상황에서 `if (false && verification.integrity === 'verified')`로 다운로드 차단 | `TimeoutError: Timeout 15000ms exceeded while waiting for event "download"` (exit 1) ➔ **즉시 사살 (KILLED)** | 원복 후 `run_real_browser_acceptance.py --scenario verified` **정상 통과 (exit 0)** |
| **갈래 2 (`mismatch`)** | 해시 불일치 상황에서 `if (integrity === 'verified' \|\| integrity === 'mismatch')`로 다운로드 허용 우회 | `AssertionError: CRITICAL: Download MUST be blocked on checksum mismatch!` (exit 1) ➔ **즉시 사살 (KILLED)** | 원복 후 `run_real_browser_acceptance.py --scenario mismatch` **정상 통과 (exit 0)** |
| **갈래 3 (`missing-header`)** | 헤더 누락 상황에서 `if (!expectedSha256) integrity = 'verified'`로 조용한 강등 우회 허용 | `AssertionError: CRITICAL: Download MUST be blocked on missing X-Content-SHA256 header!` (exit 1) ➔ **즉시 사살 (KILLED)** | 원복 후 `run_real_browser_acceptance.py --scenario missing-header` **정상 통과 (exit 0)** |

모든 돌연변이 검증 후 소스 트리를 완전히 원복하였으며, `tools/run_real_browser_acceptance.py --scenario all`로 3대 갈래가 전수 100% 통과(exit code 0)함을 최종 확인했습니다.
