...
이 문서는 가설을 제품 경로에서 확인하는 opt-in 진단과, 그 결과 뒤에만 검토할 두 정책 옵션을 설계한다. workflow·collector·제품 변형은 아직 구현하지 않았고 PostgreSQL 부하도 실행하지 않았다. coordinator가 exact SHA와 실행 범위를 명시 승인하기 전에는 로컬·hosted 모두 실행 금지다. Claude가 별도 검토 중 실수로 실행한 50동시 1회는 절차 이탈로만 기록하며, 그 수치·P95·성공률은 이 카드의 근거와 비교표에 사용하지 않는다.
...
ALTER DATABASE <quoted_disposable_db> SET deadlock_timeout = '50ms';
```
...
- 부하 시작 전 runtime session에서 `SHOW log_lock_waits=on`, `SHOW deadlock_timeout=50ms`, transaction 안에서 `SHOW lock_timeout=500ms`, `SHOW statement_timeout=2s`를 기록한다. 하나라도 다르면 `INVALID_SETUP/exit 3`이다.
...
- 각 request는 `application_name=s05lw-<opaque-run>-r<00..19>`를 쓰고 private scratch에서만 PID→`backend-N`을 매핑한다. tenant/project/run ID, DSN, SQL parameter, raw transaction/lock ID는 공개 artifact에 남기지 않는다.
...
waiter별 `segmentCount`, `sumLoggedWaitMs`, `maxSegmentWaitMs`, `holderChanges`, `tupleWaitCount`, `55P03/57014`, request completion을 함께 계산한다. raw Docker log·PID/xid map·원문 SQL은 artifact upload 전에 삭제한다.
...
| 판정 | 필수 증거 | 의미 |
|---|---|---|
| `KEY_SHARE_RESET_SUPPORTED` | 제품 legacy waiter에서 holder별 서로 다른 transactionid wait/acquired segment가 반복되고 tuple wait 0, `pgrowlocks`에 다수 Key Share+하나 No Key Update, 각 segment가 500ms 미만이면서 합계가 500ms 이상인 request 존재, `55P03=0` | FK 선행 약한 잠금→tuple FIFO 우회→holder 교체별 timeout 구간 재시작 가설을 이 wave에서 지지 |
...
| `HYPOTHESIS_CONTRADICTED` | tuple FIFO가 관측되거나 Key Share가 없는데 reset signature가 나타나거나, 같은 holder 한 segment가 명백히 500ms 초과 후 획득 | F-S05-02 가설 기각 또는 새 계측 finding; 제품 정책 변경 금지 |
...
### 옵션 B: FIFO 유지 + queue 깊이 상한

limits `FOR UPDATE`의 FIFO를 유지하되, DB lock 진입 전에 project별 동시 admission 수를 제한하고 넘는 요청은 기존 `RES-0007`/503/retryable 표면으로 fail-fast 한다.
...
옵션 A는 FIFO를 피하지만 기아·thundering herd와 fail-fast 목표가 충돌할 수 있고, 옵션 B는 FIFO를 유지하지만 별도 admission 원자성이 필요하다. 이 카드에서는 어느 것도 권고·구현하지 않는다.
...
```text
APPROVE S05-LOCK-WAIT-DIAGNOSTIC: sha=<40hex>, legacy=20x1, deadlock_timeout=50ms, pgrowlocks=true, hosted-disposable-service=true
```
