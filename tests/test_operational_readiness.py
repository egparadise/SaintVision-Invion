...
def prepared(migrated, database_url):
    ...
    ids["subject"] = (
        "oidc:" + hashlib.sha256(b"https://login.example.internal\x00operator-1").hexdigest()
    )
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        conn.execute(
            "INSERT INTO inv.business_projects(tenant_id,project_id,enabled) " "VALUES(%s,%s,true)",
            (t, p),
        )
        conn.execute(
...
def _run(ids) -> tuple[dict, int]:
    completed = subprocess.run(
        ...
        env={**os.environ, "INV_READINESS_DSN": ids["dsn"]},
        capture_output=True,
        text=True,
    )
...
def test_a_prepared_tenant_reports_ready(prepared) -> None:
    ...
    assert result["grants"]["observedRequestGrantsHold"] is True
    assert result["grants"]["mayRequestWork"] is None
    assert result["admission"]["wouldAdmit"] is None
    assert result["admission"]["observedGatesOpen"] is True
    assert code == 0
...
def test_membership_without_a_role_that_may_request_is_refused(prepared) -> None:
    ...
    assert result["grants"]["observedRequestGrantsHold"] is False
    assert result["grants"]["mayRequestWork"] is None
    assert code == 1
