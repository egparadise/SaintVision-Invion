...
def test_discovery_candidate_schema_contract_invariants() -> None:
    """Discovery candidate schema invariant: backend service and frontend contract must agree 1:1.

    Guarantees:
    1. Backend `src/saintvision/services/discovery.py:list_candidates` returns dictionary
       keys matching frontend `DiscoveryCandidate` interface in `fabricControlApi.ts`.
    2. Backend route `/v1/discovery/candidates` in `src/saintvision/api/v1/pools.py` includes
       the unverified note warning against presenting claims as measurements.
    3. Candidates are always marked with `verified: False`.
    4. Frontend mocks strictly adhere to canonical schema without fabricating verified status.
    """
    from pathlib import Path

    repo_root = Path(__file__).resolve().parents[1]
    discovery_service_path = repo_root / "src" / "saintvision" / "services" / "discovery.py"
    pools_api_path = repo_root / "src" / "saintvision" / "api" / "v1" / "pools.py"
    fabric_api_path = repo_root / "apps" / "web" / "src" / "features" / "desktop" / "fabricControlApi.ts"

    disc_text = discovery_service_path.read_text(encoding="utf-8")
    pools_text = pools_api_path.read_text(encoding="utf-8")
    fabric_text = fabric_api_path.read_text(encoding="utf-8")

    # 1. Required keys in discovery.py:list_candidates
    ...
    for key in required_backend_keys:
        assert f'"{key}":' in disc_text, f"Key '{key}' missing from discovery.py candidate dictionary"

    # 2. Invariant: 'verified' in backend must be explicitly False
    assert '"verified": False' in disc_text, "Backend candidate must hardcode verified: False before admission"

    # 3. Route /v1/discovery/candidates must include note and unverified declaration
    assert '/discovery/candidates' in pools_text
    assert "All claimed* values are self-reported by the machine and unverified." in pools_text

    # 4. Frontend DiscoveryCandidate interface in fabricControlApi.ts must specify these keys
    for key in [
        "announcementId",
        "instanceId",
        "sourceIp",
        ...
        assert f"{key}:" in fabric_text or f"{key}?:" in fabric_text, f"Key '{key}' missing from frontend DiscoveryCandidate"
