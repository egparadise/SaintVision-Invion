...
VALID = {
    "version": "operator:1",
    "allowed": [{"licensePolicy": "synthetic", "classification": "internal"}],
}
...
def test_policy_matches_trusted_worker_policy_and_is_order_independent():
    ...
    assert policy == RegistryBindingPolicy(
        "operator:1", frozenset({("synthetic", "internal"), ("second", "public")})
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({**VALID, "allowed": {}}, "Bounded registry policy allowlist required"),
        (
            {**VALID, "allowed": [VALID["allowed"][0]] * 65},
            "Bounded registry policy allowlist required",
        ),
        ...
        (
            ...
            "Bounded license/classification strings required",
        ),
        (
        ...
        (
            {
                **VALID,
                "allowed": [
                    {
                        "licensePolicy": "synthetic",
                        "classification": "internal",
                        "executionAuthorized": True,
                    }
                ],
            },
            "Exact license/classification pair required",
        ),
    ],
)
def test_invalid_policy_rejected(value, expected):
    with pytest.raises(ValueError, match=expected):
        configured_registry_policy(value)
...
def factory(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        module, "AccessTokens", lambda **kw: SimpleNamespace(tenant_id=str(uuid4()))
    )
    seen = {}
...
def test_factory_keeps_unconfigured_legacy_mode_explicit(factory):
    ...
    assert database.placement_project_semaphore_enabled is False
...
@pytest.mark.parametrize(
    "value", [None, {}, {**VALID, "allowed": []}, {**VALID, "unexpected": "sensitive-sentinel"}]
)
