...
def test_integrated_migration_keeps_both_published_histories():
    ...
    assert ordered[-1].revision == "0019_workspace_api_integration"
    ...
    assert module.downgrade_target(revisions) == "0019_workspace_api_integration"
    with pytest.raises(ValueError, match="unmerged"):
        module.chain([r for r in revisions if r != ordered[-1]])
