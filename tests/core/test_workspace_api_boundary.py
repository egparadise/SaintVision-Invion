...
def test_integrated_migration_keeps_both_published_histories():
    ...
    assert ordered[-1].revision == "0037_storage_sample_commit"
    ...
    assert set(parents["0026_business_start_merge"]) == {
    ...
    assert set(parents["0028_result_readiness_merge"]) == {
        "0027_business_api_guards",
        "0026_subject_kernel_link",
    }
    assert parents["0026_subject_kernel_link"] == "0025_workspace_tool_choice"
    assert set(parents["0030_provisioning_integrity"]) == {
        "0028_result_readiness_merge",
        "0029_run_outputs",
    }
    assert set(parents["0031_resource_offer_integrity"]) == {
        "0030_provisioning_integrity",
        "0030_apply_resource_offer",
    }
    assert parents["0030_apply_resource_offer"] == "0029_run_outputs"
    assert set(parents["0032_workspace_readiness_merge"]) == {
        "0031_resource_offer_integrity",
        "0031_workspace_input_state",
    }
    assert parents["0024_workspace_bridge"] == "0023_containment_approvals"
    assert set(parents["0033_workspace_bridge_merge"]) == {
        "0024_workspace_bridge",
        "0032_workspace_readiness_merge",
    }
    assert module.downgrade_target(revisions) == "0037_storage_sample_commit"
