...
def test_the_role_check_passes_for_an_owner_and_fails_for_a_viewer(app_sessionmaker, scene):
    with app_sessionmaker() as session:
        with session.begin():
            with tenant_scope(session, scene["tenant_a"]):
                report = readiness_service.workspace_readiness(
...
def test_an_operator_link_satisfies_the_kernel_checks(app_sessionmaker, owner_engine, scene):
    ...
    # An identity mapping does not also grant execution permission.
    assert not by_name["kernel_request_permission"]["satisfied"]
    assert report["blockedBy"] == ["operator", "requester"]


def test_the_checklist_cannot_be_used_to_probe_another_project(app_sessionmaker, scene):
    with app_sessionmaker() as session:
        with session.begin():
            with tenant_scope(session, scene["tenant_a"]):
...
def test_input_is_the_precondition_a_user_meets_after_the_others(app_sessionmaker, scene):
...
def test_the_input_reader_is_bound_to_the_session_tenant(app_sessionmaker, scene, two_tenants):
        ...
        with session.begin():
            with tenant_scope(session, scene["tenant_a"]):
                rows = session.execute(
                    text("SELECT count(*) FROM public.workspace_input_state(:t, :w)"),
