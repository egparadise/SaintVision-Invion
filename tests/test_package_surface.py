...
QUARANTINE: dict[str, str] = {
    "demo_server.py": (
        "Mock control plane for the web client. In-memory state, unsigned "
        "tokens, a hardcoded tenant, and mutating routes — cancel, "
        "reclaim-resources, resume/enqueue, approvals/approve — with no "
        "credential check. It was `server.py` until PR #13 renamed it and made "
        "server.py a real factory, which removes the likeliest accident: an "
        "operator pointing uvicorn at the obvious name. The artefact is still "
        "shipped in the installed package, so the boundary is still worth "
        "stating. Owner: Gemini."
    ),
}
...
def test_the_quarantine_list_has_not_grown_silently():
    ...
    assert set(QUARANTINE) == {"demo_server.py"}
