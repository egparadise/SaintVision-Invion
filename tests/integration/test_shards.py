...
def test_shard_manifest_requires_each_committed_evidence_and_physical_result(
    remote, storage
):
    a = remote
    shards = admissions(a)
    runtime = ShardRuntime(a.e.db, a.profile)
    runtime.enqueue(
        a.e.tenant, a.e.project, "results", shards, signing_key=a.key, splittable=True
    )
    ...
    for shard in shards:
        ...
        assert (
            runtime.status(a.e.tenant, a.e.project, "results")["resultManifest"] is None
        )
        results.complete(
            a.e.tenant,
            a.e.project,
    ...
    assert (
        len(status["resultManifest"]) == 2 and len(status["resultManifestSha256"]) == 64
    )
    assert all(s["evidenceId"] for s in status["shards"])
    assert (
        runtime.reconcile_failures(a.e.tenant, a.e.project, "results")["failedRuns"]
        == []
    )


def test_plan_cancel_checks_current_permission_and_rolls_back_all_members(
    remote, monkeypatch
):
    ...
    with pytest.raises(DomainError, match="AUTH-0030"):
        runtime.cancel(
            Principal(a.e.tenant, "outsider"), a.e.project, "cancel-all", key="denied"
        )
    ...
    first = runtime.cancel(
        a.people["requester"], a.e.project, "cancel-all", key="cancel"
    )
...
def test_nonzero_shard_result_is_reconciled_without_retry(remote):
    ...
    runtime = ShardRuntime(a.e.db, a.profile)
    runtime.enqueue(
        a.e.tenant, a.e.project, "failure", shards, signing_key=a.key, splittable=True
    )
    attempt = DeliveryQueue(a.e.db).acquire(
        a.e.tenant, command_id=shards[0].command["commandId"]
    )
    ...
    assert (
        runtime.reconcile_failures(a.e.tenant, a.e.project, "failure")["failedRuns"]
        == []
    )
