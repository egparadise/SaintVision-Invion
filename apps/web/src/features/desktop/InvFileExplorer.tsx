...
import {
  fabricObservation as catalogueApi,
  replicaStates,
  type Location,
  type ReplicaObservation,
} from '@/shared/api/fabricObservation';
...
const CatalogueInvFileExplorer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setCursor(null);
    setDetail(null);
    catalogueApi.locations(undefined, controller.signal).then(page => {
      ...
      if (!controller.signal.aborted) {
        setError('저장소 조회에 실패했습니다. 로그인과 권한을 확인하세요.');
      }
    }).finally(() => {
  ...
  const open = async (value: string) => {
    ...
    setDetail(null);
    try {
      const location = await catalogueApi.resolve(value, controller.signal);
      const observation = await catalogueApi.replicas(location, controller.signal);
      if (!controller.signal.aborted) setDetail({ location, observation });
    } catch {
      if (!controller.signal.aborted) {
        setError('파일을 조회할 수 없습니다. URI와 접근 권한을 확인하세요.');
      }
    } finally {
      if (!controller.signal.aborted) setLoading(false);
    }
  ...
  const more = async () => {
    ...
    const controller = begin();
    try {
      const page = await catalogueApi.locations(cursor, controller.signal);
      if (!controller.signal.aborted) {
  ...
  return (
    <section style={{ padding: 24, overflow: 'auto', height: '100%' }} aria-label="내 저장소">
      <h2>내 저장소</h2>
      <p>내가 등록한 활성 저장소의 파일 목록입니다. 현재 접근 가능 여부는 별도 확인이 필요합니다.</p>
      <button type="button" onClick={() => setRefresh(value => value + 1)}>목록 새로고침</button>
      <form onSubmit={event => { event.preventDefault(); void open(uri); }}>
        ...
        <button type="submit" disabled={!uri || loading}>조회</button>
      </form>
      ...
      <ul>
        {items.map(item => (
          <li key={item.locationId}>
            <button type="button" disabled={loading} onClick={() => void open(item.uri)}>{item.uri}</button>
            {' · '}{item.byteSize} bytes
          </li>
        ))}
      </ul>
      {cursor && <button type="button" disabled={loading} onClick={() => void more()}>더 보기</button>}
      {detail && (
        <article>
          <h3>{detail.location.uri}</h3>
          <p>{detail.location.byteSize} bytes</p>
          <p>SHA-256: {detail.location.checksumSha256 ?? '미확인'}</p>
          <p>현재 가용성: 미확인 · 실행 시 재검증 필요</p>
          <p>조회 시각: {detail.observation.observedAt}</p>
          <p>기록된 복제본 상태 (현재 파일 검사 결과가 아님)</p>
          <ul>
            {replicaStates.map(state => (
              <li key={state}>{state}: {detail.observation.recordedStates[state]}</li>
            ))}
          </ul>
          <p>전체 기록: {detail.observation.totalRecords}</p>
        </article>
      )}
    </section>
  );
...
const WorkspaceInvFileExplorer: React.FC<InvFileExplorerProps> = ({
  projectId,
  runId,
  checkoutId,
...
export const InvFileExplorer: React.FC<InvFileExplorerProps> = props => {
  const usesWorkspaceSurface = Boolean(
    props.projectId || props.runId || props.checkoutId || props.initialFiles?.length,
  );
  return usesWorkspaceSurface
    ? <WorkspaceInvFileExplorer {...props} />
    : <CatalogueInvFileExplorer />;
};
