...
export const NodeDetail: React.FC<NodeDetailProps> = ({ node, resourceUsage, onBack, onOpenStudio }) => {
  const hasTelemetry =
              ...
              >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px', marginTop: '4px' }}>
                  <div>
                    ...
                    <strong>{res.capacity.toLocaleString()}</strong>
                  </div>
                  <div>
                    <span style={{ color: 'var(--color-text-muted)' }}>제공량 (Offered):</span>{' '}
                    <strong>{res.offered.toLocaleString()}</strong>
                  </div>
                  <div>
        ...
        >
          ...
          >
            ...
            <div style={{ marginTop: '8px', color: 'var(--color-text-muted)' }}>
              하트비트 수신 시각: {node.heartbeatAt ? new Date(node.heartbeatAt).toLocaleString() : '미기록'} {hasTelemetry ? '(실시간 텔레메트리 연동)' : '(정적 등록 상태)'}
            </div>
          </div>
